...
export default function PokemonPopUp({ data, onClose }: PokemonPopUpProps) {
  ...
  return (
    ...
    >
      <div
        className="h-[85%] w-[90%] md:h-[75%] md:w-[50%] rounded-3xl bg-slate-200 overflow-y-auto"
      ...
      >
        ...
        <div className="flex flex-col md:flex-row h-[55%]">
          <div className="h-[50%] md:h-[100%] w-[100%] md:w-[40%] flex flex-col items-center justify-center">
            {!playgif ? (
              <img
                src={data.image}
                alt={data.name}
                className="w-[60%] h-[100%] md:w-[70%] md:h-[70%] object-fit rounded-lg"
              />
            ) : (
              <div className="w-[60%] h-[100%] md:w-[70%] md:h-[70%] flex items-center justify-center rounded-lg">
                <img
                  src={data.gif}
                  alt={data.name}
                  className="w-[60%] h-[100%] md:w-[60%] md:h-[80%] object-fit rounded-lg"
                />
              </div>
            )}
            {data.gif ? (
              <button onClick={() => setPlayGif(!playgif)}>
                see the pokemon move
              </button>
            ) : null}
          </div>
